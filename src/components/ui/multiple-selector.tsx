...
const MultipleSelector = React.forwardRef<MultipleSelectorRef, MultipleSelectorProps>(
    (
        ...
        useEffect(() => {
            if (inputValue === "") {
                // Reset options or trigger a refresh when input is cleared
                setOptions(transToGroupOption(arrayDefaultOptions, groupBy)); // Reset to default options
            }
        }, [inputValue, arrayDefaultOptions, groupBy]);
        ...
        const handleSearch = async () => {
            ...
            setOptions(transToGroupOption(results || [], groupBy));
            setIsLoading(false);
        };
    
        // Trigger the search on every input change
        useEffect(() => {
            handleSearch();
        }, [debouncedSearchTerm]);
