...
function BusinessPermitUploader({ propertyId, userId }: { propertyId: string, userId: string }) {
    ...
    const handleUpload = async () => {
        if (uppy.getFiles().length > 0) {
            ...
            const objectName = `property/${userId}/${propertyId}/business-permit/${uppy.getFiles()[0].name}`;
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen} modal={true}>
            <DialogTrigger asChild>
                ...
                >
                    <span>Upload or update business permit</span>
                </Button>
            </DialogTrigger>
            <DialogContent
