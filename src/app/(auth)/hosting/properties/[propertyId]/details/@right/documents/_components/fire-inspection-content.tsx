...
import { removeFireInspection } from "@/actions/property/propertyDocuments";

function FireInspectionContent({ fireInspection, userId, propertyId }: { fireInspection: string, userId: string, propertyId: string }) {
        ...
        <Card className="bg-background flex items-center justify-between">
            ...
            <CardContent className="px-5 py-3">
                <div className="flex items-center gap-2">
                    <Image
                        src={
                            fireInspection?.split("/").pop().split(".").pop() === "jpg" ||
                            fireInspection?.split("/").pop().split(".").pop() === "jpeg" ||
                            fireInspection?.split("/").pop().split(".").pop() === "png"
                                ? "/documents/image-document-svgrepo-com.svg"
                                : fireInspection?.split("/").pop().split(".").pop() === "docx" ||
                                  fireInspection?.split("/").pop().split(".").pop() === "doc"
                                ? "/documents/word-document-svgrepo-com.svg"
                                : fireInspection?.split("/").pop().split(".").pop() === "pdf"
                    ...
                    <div className="flex flex-col">
                        <p className="text-sm font-[500] truncate w-[300px]">
                            {fireInspection ? (
                                <span>{fireInspection?.split("/").pop()}</span>
                            ) : (
                                <span>No fire inspection certificate uploaded.</span>
                            )}
                        </p>
            ...
            <CardFooter className="gap-2 px-5 py-3">
                <Link
                    ...
                    //     async () => {};
                    // }}
                    href={fireInspection || "/hosting/properties"}
                    target="_blank"
                >
                    {isPending ? (
                        <svg
                ...
                <AlertDialog>
                    ...
                    <AlertDialogContent>
                        <AlertDialogHeader>
                            <AlertDialogTitle>Are you absolutely sure you want to delete this file?</AlertDialogTitle>
                            <AlertDialogDescription>
                                This action cannot be undone. It will permanently delete the file and remove the data from our server.
                            </AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
                            <AlertDialogCancel
                                disabled={isPending}
                                className={cn(buttonVariants({ size: "sm", variant: "secondary" }), "border-none shadow-lg")}
                            >
                            ...
                            <AlertDialogAction
                                ...
                                onClick={async () => {
                                    await startTransition(async () => {
                                        await removeFireInspection(propertyId, fireInspection, userId);
                                        router.refresh();
                                    });
                                }}
                                disabled={isPending || !fireInspection}
                            >
                                <div className="flex items-center">
                                    {isPending && (
                                        <svg
